...
func (s *Snapshot) buildRecentActivity() ([]snapshotExecEvent, recentActivity) {
	if s.store == nil {
...
func convertStorageExecEvents(storageEvents []*storage.Event) []snapshotExecEvent {
	execs := make([]snapshotExecEvent, 0, len(storageEvents))
	for _, sev := range storageEvents {
		...
		case *events.ExecEvent:
			...
			execs = append(execs, snapshotExecEvent{
